...
class NotificationViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        """Returns a list of notifications with unread first, then read notifications."""
        unread_notifications = Notification.objects.filter(
            user=request.user, is_read=False).order_by('-created_at')
        read_notifications = Notification.objects.filter(
            user=request.user, is_read=True).order_by('-created_at')
        
        unread_serializer = self.get_serializer(unread_notifications, many=True)
    ...
    def update(self, request):
        """Mark a notification as read and manage the max limit of read notifications."""
